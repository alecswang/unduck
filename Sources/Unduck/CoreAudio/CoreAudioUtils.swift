...
/// Convert a decibel target into the scalar the device wants. The scalar->dB curve
/// is device-specific and steeply non-linear (built-in speakers: 0.1875 = -36 dB,
/// 0.375 = -24.6 dB, 0.5 = -18.6 dB), so it must be asked for, never modelled.
func deviceScalar(for decibels: Float32, on device: AudioObjectID) -> Float32? {
    for element in [kAudioObjectPropertyElementMain, UInt32(1)] {
        var addr = address(kAudioDevicePropertyVolumeDecibelsToScalar, kAudioObjectPropertyScopeOutput, element)
        guard AudioObjectHasProperty(device, &addr) else { continue }
        var value = decibels
        var size = UInt32(MemoryLayout<Float32>.size)
        if AudioObjectGetPropertyData(device, &addr, 0, nil, &size, &value) == noErr { return value }
    }
    return nil
}
